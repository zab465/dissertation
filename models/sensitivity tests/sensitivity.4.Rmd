...
# Prepare the data
X_ex <- model.matrix(fdextort ~ combined_duration + combined_state_sponsor + crim_degr_py +combined_size + pure_religious + pure_secular, 
...
# Prepare the data
X_rob <- model.matrix(fdrob ~ combined_duration + combined_state_sponsor + crim_degr_py +combined_size + pure_religious + pure_secular, 
...
# Prepare the data
X_smug <- model.matrix(fdsmuggl ~ combined_duration + combined_state_sponsor + crim_degr_py +combined_size + pure_religious + pure_secular, 
...
# Prepare the data
X_drug <- model.matrix(combined_drug_trafficking ~ combined_duration + combined_state_sponsor + crim_degr_py +combined_size + pure_religious + pure_secular, 
...
# Prepare the data
X_kidnap <- model.matrix(combined_kidnap ~ combined_duration + combined_state_sponsor + crim_degr_py +combined_size + pure_religious + pure_secular, 
...
stargazer(extortion_lm1,rob_lm1, smug_lm1, kidnap_lm1,drug_lm1,
					...
					title = "Table 5: Regressions of Terrorist Crimes on Organization Level Factors with Purist Ideologies", 
					dep.var.labels = c("Extortion", "Robbery", "Smuggling", "Kidnapping", "Drug Trade"), 
					dep.var.caption = "Type of Funding Crime", out = "sensitivity.4.1.html",
...
table_vif <- knitr::kable(vif_table, caption = "Table 6: VIF Scores of Regressions of Terrorist Funding Crimes on Organization Level Factors") %>%
  kable_styling(bootstrap_options = c("striped", "hover"), full_width = TRUE, position = "center")


save_kable(table_vif, "sensitivity.4.2.html")
```
...
grid <- grid.arrange(plot1, plot2, plot3, plot4, plot5, nrow = 3, ncol = 2, 
										 top = textGrob("Figure 4: Purist Ideology Regressions - Residuals vs. Fitted Plots", 
                                   gp = gpar(fontsize = 20, fontface = "bold")))
										 
ggsave("sensitivity.4.3.pdf", grid, width = 10, height = 10)
```
